...
struct InternalDefaultLiveMapTests {
    ...
    struct GetTests {
        // @spec RTLM5c
        @Test(arguments: [.detached, .failed] as [AblyPlugin.RealtimeChannelState])
        func getThrowsIfChannelIsDetachedOrFailed(channelState: AblyPlugin.RealtimeChannelState) async throws {
    ...
    struct AccessPropertiesTests {
        ...
        // @spec RTLM13b
        @Test(arguments: [.detached, .failed] as [AblyPlugin.RealtimeChannelState])
        func allPropertiesThrowIfChannelIsDetachedOrFailed(channelState: AblyPlugin.RealtimeChannelState) async throws {
            let logger = TestLogger()
            let map = InternalDefaultLiveMap.createZeroValued(objectID: "arbitrary", logger: logger, userCallbackQueue: .main, clock: MockSimpleClock())
    ...
    struct SetTests {
        // @spec RTLM20c
        @Test(arguments: [.detached, .failed, .suspended] as [AblyPlugin.RealtimeChannelState])
        func throwsErrorForInvalidChannelState(channelState: AblyPlugin.RealtimeChannelState) async throws {
            let logger = TestLogger()
            let map = InternalDefaultLiveMap.createZeroValued(objectID: "arbitrary", logger: logger, userCallbackQueue: .main, clock: MockSimpleClock())
            let coreSDK = MockCoreSDK(channelState: channelState)
