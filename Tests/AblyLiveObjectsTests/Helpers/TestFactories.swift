...
struct TestFactories {
    ...
    static func counterCreateOperation(
        ...
        objectOperation(
            ...
            objectId: objectId,
            counterCreate: CounterCreate(count: count.map { NSNumber(value: $0) }),
        )
    }

    /// Creates a CounterInc
    static func counterInc(number: Int = 10) -> CounterInc {
        CounterInc(number: NSNumber(value: number))
    }
